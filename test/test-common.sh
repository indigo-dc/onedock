...
function wait_for_timeout_equal {
    ...
    t1=$(date +%s)
    while [ "$(eval $CONDITION)" == "$VALUE" ] && \
        [ $(( $(date +%s) - $t1 )) -lt $TIME ]; do
        spin
        sleep $STEP
    done
}
...
function create_vm {
    ...
    # Check pending state
    CMD="onevm show -x $VMID |\
        /var/lib/one/remotes/datastore/xpath.rb /VM/STATE"
    wait_for_timeout_equal 60 1 "$CMD" "1"

    progress

    # Wait a little more, until running state
    wait_for_timeout_nequal 60 1 "onevm show -x $VMID |\
        /var/lib/one/remotes/datastore/xpath.rb /VM/STATE" "3"
    ...
    IP=$(onevm show -x $VMID |\
        /var/lib/one/remotes/datastore/xpath.rb /VM/TEMPLATE/NIC/IP)
