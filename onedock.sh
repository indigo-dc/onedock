...
function get_imagename_from_file {
    ...
    [ $? -ne 0 ] && echo "file $FILE is not a docker image (does\
        not have the proper format)" && return 1
    IMAGE_COUNT=$(echo "$CONTENT" | jq '.[] | keys | length')
    [ "$IMAGE_COUNT" != "1" ] && echo "file $FILEcontains more than one\
        image or other error has happened: $IMAGE_COUNT" && return 1
