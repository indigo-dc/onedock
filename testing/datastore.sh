...
{
    RESULT=$($TESTING_LOCATION/datastore/onedock/clone docker://dockerimage:0 wn1:/var/lib/one//datastores/0/0/disk.0 0 100)
    assertTrue "texto $RESULT" "[ $? -eq 0 ]"
}

NOT_SUPPORTED_testRM() {
    TEMPLATE=$(echo $DSACTION_TEMPLATE_PLAINTXT | sed 's/%%PATH%%/docker:\/\/ubuntu:latest/' | base64)
...
testCP_DockerHub() {
    ...
    assertSame "failed to test CP" "docker://$LOCAL_SERVER/dockerimage:0 0" "$RESULT"
}

testCP_DockerFile() {
    TMPDIR="/tmp/t$(date +%s)"
    PATTERN=$(echo $TMPDIR | sed 's/\//\\\//g')
    TEMPLATE=$(echo "$DSACTION_TEMPLATE_PLAINTXT" | sed "s/%%PATH%%/$PATTERN.tar/" | base64)
    mkdir -p $TMPDIR
    cat > $TMPDIR/repositories <<\EOF
{"ec4docker":{"frontend":"fe6d54a00e6775012430ed4a6f14dd95b25dad0b9d58282f1f92ab0a8a7fb888"}}
EOF
...
testMonitor() {
    ...
    assertTrue "texto $RESULT" "[ $? -eq 0 ]"
    assertSame "failed to find USED_MB" "$(echo $RESULT | grep 'USED_MB=' -c)" "1"
    assertSame "failed to find FREE_MB" "$(echo $RESULT | grep 'FREE_MB=' -c)" "1"
    assertSame "failed to find TOTAL_MB" "$(echo $RESULT | grep 'TOTAL_MB=' -c)" "1"
}
